...
ERB 加载解析目前串行(已评估、未实施):`Preload` 已并行读文件;`loadErb` / `ParseScript` 仍串行。并行前需先处理 `Depth` 调用图发现顺序,以及 `scaningLine` / `ParserMediator` 等共享状态。
...
- 来自 `dotnet-modernization` 的衔接：装载循环内泵消息的 `Application.DoEvents()`（`ErhLoader.analyzeSharpDimLines` / `ErbLoader.ParseFunctionWithCatch`）允许界面操作在装载中途重入，并行化后与共享状态风险叠加；改造时需一并移除该泵或改为明确的取消机制。
...
- 来自 issues/03 的衔接：解析期标识符驻留（span 键查重复用 AST 常驻字符串）的评估延至本票实施时一并处理,评估结论与并发安全约束见 issues/03 的「唯一有实际空间的方向」一节。

## 影响

- 装载期语义变化:同名函数裁决规则由兼容配置决定(默认值待 triage);事件函数同桶执行顺序需确定性规则。
- 装载循环 `Application.DoEvents()` 泵需一并移除或改为明确取消机制。
- 标识符驻留评估随本票实施一并处理(见 issues/03)。
