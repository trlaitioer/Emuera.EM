...
5. DllImport → LibraryImport 决策(issues/05):无 AOT 需求,维持现状
6. C# 语言特性现代化策略决策(issues/06):C# 14 已随 TFM 默认生效,建议整体不立项、按需(on-touch)采纳

范围切分:UI 层的 WinForms 升级评估与显示改造(DPI 缩放、等宽网格渲染)已于 2026-08-30 切至独立 tracker `.scratch/ui-modernization/`,本 tracker 不再跟踪;`Application.DoEvents()`(ErhLoader / ErbLoader 装载解析循环内泵 UI)不单独立票,并入 `performance-optimization/issues/04`(解析并行化)处理,该票正文已列为本票风险点。

各项按各自 ticket 验收;决策记录票以文档定案。

### 已核查无接触面(2026-08-30 扫描,避免重复排查)
...
各项处置限于网络栈、存档读取健壮性、URL 打开路径与语言风格策略;存档格式与脚本语义不变。
