...
17 个扩展用例(FORM 字符串/表达式/常用函数,总计 38 个,全绿确认)。按用户要求,同类测试合并进对应组件的测试文件,并按源码路径镜像组织(相对 `Emuera.Tests/`):
...
3. **函数标识符表达式**的解析需 `GlobalStatic.IdentifierDictionary`,其构造依赖 `VariableData(GameBase, ConstantData)`——与 `StrForm` 同属运行时初始化范畴,**按用户决策推迟**(函数本身纯计算已直调覆盖);另 `DECIMAL_STRING` 未在内建注册表中(自定义函数)
