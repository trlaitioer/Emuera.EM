...
在 Phase 1 基础上扩展 17 个用例,覆盖 FORM 字符串构造、表达式解析、常用函数。完成标准:全绿确认(总计 38 例);未做 `Emuera/` 代码改动。

## 方案

fixture 采用内联字符串常量(行数 <10 内联、不加来源注释);同类测试合并进对应组件的测试文件,并按源码路径镜像组织(相对 `Emuera.Tests/`):
...
1. PRINTFORML 的 FORM 参数由 `AnalyseFormattedString` 解析(见 `Instraction.Child.cs` 的 PRINTFORML 指令分支),主 `Analyse` 解析整行遇裸 `{` 会报 UnexpectedCharacter——测试须对 FORM 参数文本直接调 `AnalyseFormattedString`
...
3. 函数标识符表达式的解析需 `GlobalStatic.IdentifierDictionary`,其构造依赖 `VariableData(GameBase, ConstantData)`——与 `StrForm` 同属运行时初始化范畴,**决策:推迟**(函数本身纯计算已直调覆盖);另 `DECIMAL_STRING` 未在内建注册表中(自定义函数)
...
`LogicalLineParser` 与运行时初始化范畴用例由 issues/03 承接。
