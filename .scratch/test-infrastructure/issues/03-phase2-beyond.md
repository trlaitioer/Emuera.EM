...
- `LogicalLineParser` 测试(原判断"需先解决 `EmueraConsole` STA 线程"不成立,见方案)
- eraBasic 脚本级冒烟测试

完成标准:`dotnet test Emuera.Tests/Emuera.Tests.csproj -c Debug-NAudio` 全绿;零 `Emuera/` 改动;StrForm 部分全绿后,`../nullable-migration/issues/01-core-runtime.md` 的 StrForm 联动项解锁。
...
1. AnalysisMode 子进程冒烟:`Program.Main` 在 AnalysisMode 下仍 `Application.Run` 起主窗口且无自动退出逻辑,不可行,放弃;冒烟改为进程内逐行 `ParseLine`

## 影响

测试套件覆盖扩至 StrForm/变量解析/LogicalLineParser/脚本级冒烟,`Emuera/` 零改动;Nullable 迁移的 StrForm 联动项随之解锁。
