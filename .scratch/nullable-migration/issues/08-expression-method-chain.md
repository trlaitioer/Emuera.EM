...
- `VariableTerm`/`VariableStrArgTerm`(`AExpression` 子类,定义在 Variable/)的签名契约由 issues/10 承接;本票如需先改其签名,在 Comments 记录
...
- 建议在 07(词法输入契约)与 10(VariableTerm 契约)之后执行,不强制
- `FunctionMethod.argumentTypeArray` 已为 `Type[]?`(issues/01 完成)

## Comments

### 2026-08-29

- 新建:按调用链重切差分,原 issues/01 的 Creator.Method.cs、issues/02 的 Expression/、issues/04 的 Function/ 剩余部分归入本票。
