...
按 spec.md 通用方法逐文件启用;为本链警告修改票外文件签名时在 Comments 记录。Word/SubWord 触点的修改方向已经 issues/01(StrForm 修订)验证。

## 备注

- 下游:08(表达式以 WordCollection 为输入)、10(ParserMediator、UserDefinedVariable 引用解析类型)、11(ErbLoader)、12(UI 的 EmueraConsole/HtmlManager/ButtonStringCreator/DebugDialog 调用 LexicalAnalyzer)
- 纯解析,无 UI/Sound 依赖,与声音后端配置无关

## Comments

### 2026-08-29

- 新建:按调用链重切差分,原 issues/04 中 Parser/ 部分与 LogicalLine.cs、EraStreamReader.cs 归入本票。
