...
- 先于 issues/09(Instraction.Child 直接使用 UI 打印类型、Sound、Config、Lang)与 issues/12;其余链票可并行参考已定决策
- 与 issues/05(方法调用链元素可空)并列先行,两票无相互依赖

## Comments

### 2026-08-29

- 新建:按调用链重切差分(见 spec.md 迁移方法),原按文件目录分摊的模式下横切契约无归属,集中为本票。
